#!/bin/bash
# macOS installer for Codagotchi daemon (Python + bleak + launchd).
# Uses LaunchAgents for autostart.
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SERVICE_LABEL="com.rachelworld.codagotchi"
...
DAEMON_PY="$SCRIPT_DIR/daemon/codagotchi_daemon.py"
LOG_DIR="$HOME/Library/Logs"
LOG_OUT="$LOG_DIR/codagotchi.out.log"
LOG_ERR="$LOG_DIR/codagotchi.err.log"

echo "=== Codagotchi macOS install ==="
echo ""
...
echo "  3. Click 'Connect' next to 'Codagotchi'."
echo "  4. The daemon will discover it within ~30 s and start polling."
echo ""
echo "Useful commands:"
echo "  launchctl list | grep codagotchi       # check it's running"
