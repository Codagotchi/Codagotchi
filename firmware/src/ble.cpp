...
static void start_advertising() {
    ...
    //   flags (3) + appearance (4) + HID service 0x1812 (4) + name "Codagotchi" (12)
...
void ble_init(void) {
    ...
    hid_dev->setManufacturer("rachelworld");
...
// Map a printable ASCII char to its US-layout HID usage code (no modifier).
// Returns 0 for characters we don't support — caller skips those.
static uint8_t ascii_to_hid(char c) {
    if (c >= 'a' && c <= 'z') return 0x04 + (uint8_t)(c - 'a');
    if (c >= 'A' && c <= 'Z') return 0x04 + (uint8_t)(c - 'A');  // sent unshifted → lowercase
...
// Per-report delay. Must exceed the BLE connection interval so each notify is
// actually transmitted before setValue() overwrites the report — otherwise the
// release is clobbered and the host auto-repeats the (still-held) key. The C6's
// single core needs comfortable headroom here.
#define KBD_REPORT_MS 30

void ble_keyboard_type(const char* s) {
    if (state != BLE_STATE_CONNECTED || !input_kbd || !s) return;
    // Start from a known-released state and let the link settle, so the first
    // keystroke isn't doubled (it can race a preceding full-screen render).
    uint8_t idle[8] = {0};
    input_kbd->setValue(idle, sizeof(idle));
    input_kbd->notify();
    delay(KBD_REPORT_MS * 2);
    for (const char* p = s; *p; ++p) {
        uint8_t k = ascii_to_hid(*p);
        if (!k) continue;
        // Uppercase letters need the Shift modifier (the keycode itself is the
        // same as the lowercase letter).
        uint8_t mod = (*p >= 'A' && *p <= 'Z') ? 0x02 : 0;
        uint8_t report[8] = {mod, 0, k, 0, 0, 0, 0, 0};
        input_kbd->setValue(report, sizeof(report));
        input_kbd->notify();
        delay(KBD_REPORT_MS);
        uint8_t release[8] = {0};
        input_kbd->setValue(release, sizeof(release));
        input_kbd->notify();
        delay(KBD_REPORT_MS);
    }
}
