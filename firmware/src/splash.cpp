...
// ---- Mini-canvas widget (idle breathe animation embedded in other screens) ----
...
static void mini_render_frame(const splash_anim_def_t* anim, MiniState* s) {
    if (!s->buf) return;
    const uint8_t* cells = anim->frames[s->frame];
    const uint16_t* palette = anim->palette;
    for (int gy = 0; gy < GRID; gy++) {
        for (int gx = 0; gx < GRID; gx++) {
            uint8_t code = cells[gy * GRID + gx];
            uint16_t color = (palette && code < SPLASH_PALETTE_SIZE) ? palette[code] : COL_EMPTY;
            for (int dy = 0; dy < s->cell; dy++) {
                for (int dx = 0; dx < s->cell; dx++) {
                    s->buf[(gy * s->cell + dy) * s->cw + gx * s->cell + dx] = color;
                }
            }
        }
    }
...
lv_obj_t* splash_mini_create(lv_obj_t* parent, int size_px) {
    if (SPLASH_ANIM_COUNT == 0) return nullptr;

    int c = size_px / GRID;
    if (c < 1) c = 1;
    int cw = GRID * c;

    MiniState* s = (MiniState*)heap_caps_malloc(sizeof(MiniState),
...
#endif
    s->buf = (uint16_t*)heap_caps_malloc((size_t)cw * cw * 2, buf_caps);
    if (!s->buf) { heap_caps_free(s); return nullptr; }

    s->frame   = 0;
    s->cell    = c;
    s->cw      = cw;
    s->next_ms = lv_tick_get() + splash_anims[0].holds[0];

    mini_render_frame(&splash_anims[0], s);

    lv_obj_t* mini = lv_canvas_create(parent);
    lv_canvas_set_buffer(mini, s->buf, cw, cw, LV_COLOR_FORMAT_RGB565);
    lv_obj_set_size(mini, cw, cw);
    lv_obj_set_user_data(mini, s);

    return mini;
}

void splash_mini_tick(lv_obj_t* mini) {
    if (!mini || SPLASH_ANIM_COUNT == 0) return;
    MiniState* s = (MiniState*)lv_obj_get_user_data(mini);
    if (!s) return;

    uint32_t now = lv_tick_get();
    if (now < s->next_ms) return;

    const splash_anim_def_t* anim = &splash_anims[0];
    s->frame = (s->frame + 1) % anim->frame_count;
    s->next_ms = now + anim->holds[s->frame];
    mini_render_frame(anim, s);
    lv_obj_invalidate(mini);
}
