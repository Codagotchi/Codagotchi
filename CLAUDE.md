...
**Codagotchi** — ESP32 desk display for AI API usage monitoring (Claude Code first, multi-provider later). Device BLE name: "Codagotchi". GitHub: github.com/rachelworld/codagotchi.

ESP32-S3 firmware for a desk-side AI usage monitor. Each supported
...
If `pio` isn't on PATH: try `~/.platformio/penv/bin/pio` or `brew install platformio`.

Device path on macOS is `/dev/cu.usbmodem*` — the ESP32-S3 native USB-JTAG (no boot-mode dance needed). (Linux support is untested and not included.)

## QA your own UI changes — don't ask the user

The firmware ships a `screenshot` serial command that dumps the LVGL framebuffer. `./screenshot.sh out.png [port]` captures a PNG sized to the active display (480×480 or 368×448). **Use this on every UI iteration** — Read the PNG with the Read tool, verify the change visually, iterate. Script defaults to the macOS port (`/dev/cu.usbmodem101`) and falls back to pio's bundled Python if pyserial isn't on the system Python.
...
macOS daemon: `daemon/codagotchi_daemon.py` (Python + `bleak`). Reads the Claude
OAuth token from the Keychain (service `Claude Code-credentials`) and the Codex
token from `~/.codex/auth.json`, polls the provider usage APIs, and sends JSON over
BLE GATT to the ESP32. Installed as a launchd agent by `install-mac.sh`
(`com.rachelworld.codagotchi`); logs land in `~/Library/Logs/codagotchi.{out,err}.log`.
Providers are auto-detected (Claude always; Codex if `~/.codex/auth.json` exists) or
forced via `CODAGOTCHI_PROVIDERS=claude,codex`. **Linux is not supported.**

**Discovery & resilience:**

- Connects by name (`"Codagotchi"`) via `BleakScanner.discover`, caches the
  resolved address at `~/.config/codagotchi/ble-address` (`SAVED_ADDR_FILE`).
  ESP32 BLE addresses are factory-burned per-chip, so swapping any board
  invalidates the cache — delete the file to force a re-scan.
- `POLL_INTERVAL=60`, `STATE_TICK=0.5`. The loop wakes every 0.5s to push fast
  active-state changes; full API polls happen every 60s OR when the ESP fires a
  refresh request.
...
- `...0004` REQ — firmware fires `0x01` notify in `onSubscribe` if `has_received_data`
  is false. Daemon subscribes with `bleak`'s `start_notify(REQ_CHAR_UUID, …)`, which
  sets an `asyncio.Event` the poll loop awaits — triggering an immediate re-poll.
